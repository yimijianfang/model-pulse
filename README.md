...
- 支持单模型和全部模型的快速测试，以及专业标准基准。
- 标准基准先预热一次，再执行三轮正式采样；多模型按轮次随机交错，降低固定执行顺序造成的偏差。
- 展示生成阶段吞吐、字符吞吐、端到端输出速度、首字延迟、总耗时和输出 token 数。
- 汇总最近五个样本的 P50、P95、波动系数、成功率和稳定性。
- 汇总最近 24 小时成功率，并标识取消、失败、配置已变化和估算用量。
- 测试可随时停止；失败请求不会自动重试，避免重复计费。
- 所有模型共用测试提示词、输出上限和超时设置，确保对比口径一致。
- 内置数字任务会校验输出完整性；无效输出和估算 token 不参与专业排名。

> 标准基准对每个模型发起 4 次请求（1 次预热 + 3 次正式采样）。应用会在开始前展示预计请求数，由用户确认后执行。
...
- 通过 ECharts 展示生成吞吐、字符吞吐、端到端输出速度、首字延迟和总耗时趋势。
- 可按模型、时间范围、运行模式、状态、错误类型和指标筛选，单次最多加载最近 3,000 条记录。
- 支持导出不含密钥、请求头、测试提示词和完整输出内容的 CSV 或 JSON 诊断数据。
...
```text
生成吞吐 = 服务商返回的输出 token 数 ÷ 首个文本增量至完成事件的时间
字符吞吐 = Unicode 字符数 ÷ 首个文本增量至完成事件的时间
端到端有效吞吐 = 服务商返回的输出 token 数 ÷ 请求总耗时
```

端到端总耗时包含首字等待。若服务商未返回输出用量，ModelPulse 会用 Unicode 字符数除以 4 后向上取整进行粗略估算，并明确标记；估算结果不会参与 token 排名。字符吞吐可辅助比较 tokenizer 不同的模型。定时排名至少需要三个有效样本，并使用最近五个有效样本的中位数；不同测试配置不会混排。
