...
class Store {
  ...
  history({modelId='',days=7,limit=1000,mode='',status='',errorCategory=''}={}) {const since=new Date(Date.now()-days*86400000).toISOString();const where=modelId?'timestamp>=? AND model_id=?':'timestamp>=?',scan=Math.min(10000,Math.max(limit,limit*4));const rows=this.rows(`SELECT id,round_id,data FROM results WHERE ${where} ORDER BY timestamp DESC LIMIT ?`,modelId?[since,modelId,scan]:[since,scan]).map(r=>({...JSON.parse(r.data),id:r.id,roundId:r.round_id}));return rows.filter(row=>(!mode||row.mode===mode)&&(!status||row.status===status)&&(!errorCategory||row.errorCategory===errorCategory)).slice(0,limit);}
